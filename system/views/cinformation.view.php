...
class information {
	...
	private function showInfoProfile($sql, $personal) {
		...
		while ($row = $run->fetch_row()) {
			...
			echo '<div style="float: left;"><br> <h2>Registered Program Information:</h2><br>
			...
			$sqls = "SELECT DISTINCT `courses`.ID, `CourseDescription`, `courses`.Name as NAME, `periods`.`Year`
			...
			while ($fetchw = $runo->fetch_assoc()) {
				...
				$c++;
				if($c == 1){
					echo'<tr><td colspan="2"><p><br><b>COURSE REGISTRATION: '.$years.'</b><br>';
				}
				echo'<li>'.$c.' - <b>'.$fetchw['NAME'].'</b>  <i>'.$fetchw['CourseDescription'].'</i> - <a href="' . $this->core->conf['conf']['path'] . '/register/course/delete/'.$fetchw['ID'].'?userid='.$uid.'">X</a></li>';
			}
			...
			while ($fetch = $run->fetch_assoc()) {
				...
				echo '<div style="float: left;">
							<p><br><b>BOARDING INFORMATION: </b><br>
				<table width="500" height="" border="0" cellpadding="0" cellspacing="0">
				<tr>
