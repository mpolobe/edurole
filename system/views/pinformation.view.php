...
class information {
	...
	private function byprogramInformation($program, $listType, $year, $mode) {
		if ($program != "" && is_numeric($program)) {
			...
			$sql ="SELECT * FROM `basic-information`, `student-program-link`, `programmes` 
				WHERE 
				`basic-information`.`ID` = `student-program-link`.`StudentID`
				AND `programmes`.`ID` = `student-program-link`.Major
				AND `basic-information`.`ID` LIKE '" . $year . "%'
				AND `basic-information`.`StudyType` = '" . $mode . "'
				AND `programmes`.ID = '$program'
				OR
				`basic-information`.`ID` = `student-program-link`.`StudentID`
				AND `programmes`.`ID` = `student-program-link`.Minor
				AND `basic-information`.`ID` LIKE '" . $year . "%'
				AND `basic-information`.`StudyType` = '" . $mode . "'
				AND `programmes`.ID = '$program'";
		}
	...
	private function showInfoProfile($sql, $personal) {
		...
		while ($row = $run->fetch_row()) {
			...
			<div class="segment" style="width: 500px;">Student course information</div>
			...
			echo'<tr><td colspan="2"><p><br><b>Selected courses: </b><br>';

			$sqls = "SELECT DISTINCT `courses`.Name, `courses`.CourseDescription, `courses`.ID
			FROM `course-electives`,`courses` 
			WHERE `course-electives`.StudentID  = '$uid' AND `course-electives`.CourseID = `courses`.ID
			AND `course-electives`.Approved IN (1)";

			$runo = $this->core->database->doSelectQuery($sqls);

			while ($fetchw = $runo->fetch_assoc()) {
				echo'<li>'.$fetchw['Name'].'  - <i>'.$fetchw['CourseDescription'].'</i> - <a href="' . $this->core->conf['conf']['path'] . '/register/course/delete/'.$fetchw['ID'].'?userid='.$uid.'">Delete</a></li>';
			}
			...
			echo '<div style="float: left;">
			<div class="segment"  style="width: 500px;">Contact information</div>
			...
			while ($fetch = $run->fetch_row()) {
				...
				echo '<div style="float: left;"><div class="segment">Emergency information</div>
				<table width="500" height="" border="0" cellpadding="0" cellspacing="0">
				  <tr>
