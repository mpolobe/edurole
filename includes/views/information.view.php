...
class information {
	...
	if(isset($studentlastname) || isset($studentfirstname)) {
		...
		if($studentfirstname==""){ $studentfirstname="%"; }
		if($studentlastname==""){  $studentlastname="%"; }
	
		$sql ="SELECT * FROM `basic-information` WHERE `Surname` LIKE '".$studentlastname."' AND `Firstname` LIKE '".$studentfirstname."'";			$run = $this->core->database->doSelectQuery($sql);
		...
		if($study!="" && is_numeric($study)){ 		$sql ="SELECT * FROM `basic-information`, `student-study-link` WHERE `student-study-link`.StudentID = `basic-information`.ID AND StudyID = '".$study."'";	}
		if($program!="" && is_numeric($program)){  	$sql ="SELECT * FROM `basic-information`, `student-program-link` WHERE `student-program-link`.StudentID = `basic-information`.GovernmentID AND Major = '".$program."' OR `student-program-link`.StudentID = `basic-information`.ID AND Minor = '".$program."'";	}
    ...
    function showInfoProfile($run){
		...
		$results = TRUE;
		$firstname = $row[0]; 		$middlename = $row[1];
		$surname = $row[2]; 		$sex = $row[3];
		$uid = $row[4]; 		$nrc = $row[5];
		$dob = $row[6];			$pob = $row[7];
		$nationality = $row[8];
	
		$streetname = $row[9]; 		$postalcode = $row[10];
		$town = $row[11]; 		$country = $row[12];
		$homephone = $row[13]; 		$mobilephone = $row[14];
	
		$disability = $row[15];
		$disabilitytype = $row[16];	$email = $row[17];
		$maritalstatus  = $row[18];	$studytype = $row[22];
		$studentstatus  = $row[23]; 	$approved  = $row[23];
